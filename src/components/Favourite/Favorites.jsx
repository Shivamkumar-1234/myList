...
import { useNavigate, useOutletContext } from "react-router-dom"
import axios from "axios"
import { AlertTriangle, Bookmark, Calendar, Heart } from "lucide-react"
import { toast } from "react-toastify"
import "./Favorites.css"
...
const Favorites = () => {
  ...
  if (error) {
    return (
      <div className={`favorites-crunchyroll-page ${isSidebarCollapsed ? "favorites-page--sidebar-collapsed" : ""}`}>
        <div className="favorites-cr-container">
          <div className="favorites-error-state">
            <AlertTriangle className="favorites-error-icon" />
            <h2>Something went wrong</h2>
            <p className="favorites-error-message">{error}</p>
            <button className="favorites-cr-button favorites-primary" onClick={() => window.location.reload()}>
              Try Again
            </button>
          </div>
        </div>
          ...
          <div className="favorites-header-content">
            <div className="favorites-header-left">
              ...
              <div className="favorites-page-meta">
                <span className="favorites-meta-item">
                  <Bookmark className="favorites-meta-icon" />
                  {!user ? "Please login to view your list" : isLoading ? "Loading..." : `${favorites.length} titles`}
                </span>
                <span className="favorites-meta-item">
                  <Calendar className="favorites-meta-icon" />
                  Last updated today
          ...
          {!user ? (
                ...
                >
                  <div className="favorites-card-poster">
                    <div className="favorites-poster-container">
                      ...
                      {anime.year && <div className="favorites-year-badge">{anime.year}</div>}
                    </div>
                  </div>
                  <div className="favorites-card-content">
                    <h3 className="favorites-card-title" title={anime.title}>
            ...
            <div className="favorites-empty-state">
              <div className="favorites-empty-content">
                <Heart className="favorites-empty-icon" />
                <h3>No favorites yet</h3>
                <p>Start building your favorites list by adding anime you love!</p>
                <button className="favorites-cr-button favorites-primary" onClick={() => navigate("/search-page")}>
                  Browse Anime
                </button>
              </div>
            </div>
